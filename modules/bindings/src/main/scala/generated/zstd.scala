...
object enumerations:
  ...
  /** [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  ...
  object ZSTD_ResetDirective extends CEnumU[ZSTD_ResetDirective]:
    ...
    inline def getName(inline value: ZSTD_ResetDirective): Option[String] =
      inline value match
        ...
        case ZSTD_reset_session_and_parameters =>
    ...
    inline def getName(inline value: ZSTD_cParameter): Option[String] =
      inline value match
        ...
        case ZSTD_c_enableLongDistanceMatching =>
          Some("ZSTD_c_enableLongDistanceMatching")
...
object aliases:
  ...
  object ZSTD_CStream:
    ...
    extension (v: ZSTD_CStream) inline def value: ZSTD_CCtx = v
  ...
  object ZSTD_DStream:
    ...
    extension (v: ZSTD_DStream) inline def value: ZSTD_DCtx = v
  ...
  object max_align_t:
    ...
    extension (v: max_align_t) inline def value: Double = v
  ...
  object ptrdiff_t:
    ...
    extension (v: ptrdiff_t) inline def value: CLongInt = v
  ...
  object rsize_t:
    ...
    extension (v: rsize_t) inline def value: CUnsignedLongInt = v
  ...
  object size_t:
    ...
    extension (v: size_t) inline def value: CUnsignedLongInt = v
  ...
  object wchar_t:
    ...
    extension (v: wchar_t) inline def value: CInt = v
end aliases
...
object structs:
  ...
  object ZSTD_bounds:
    given _tag: Tag[ZSTD_bounds] =
      Tag.materializeCStruct3Tag[size_t, CInt, CInt]
    def apply()(using Zone): Ptr[ZSTD_bounds] =
      scala.scalanative.unsafe.alloc[ZSTD_bounds](1)
    def apply(error: size_t, lowerBound: CInt, upperBound: CInt)(using
        Zone
    ): Ptr[ZSTD_bounds] =
      val ____ptr = apply()
      (!____ptr).error = error
  ...
  object ZSTD_inBuffer:
    given _tag: Tag[ZSTD_inBuffer] =
      Tag.materializeCStruct3Tag[Ptr[Byte], size_t, size_t]
    def apply()(using Zone): Ptr[ZSTD_inBuffer] =
      scala.scalanative.unsafe.alloc[ZSTD_inBuffer](1)
    def apply(src: Ptr[Byte], size: size_t, pos: size_t)(using
        Zone
    ): Ptr[ZSTD_inBuffer] =
      val ____ptr = apply()
      (!____ptr).src = src
  ...
  object ZSTD_inBuffer_s:
    given _tag: Tag[ZSTD_inBuffer_s] =
      Tag.materializeCStruct3Tag[Ptr[Byte], size_t, size_t]
    def apply()(using Zone): Ptr[ZSTD_inBuffer_s] =
      scala.scalanative.unsafe.alloc[ZSTD_inBuffer_s](1)
    def apply(src: Ptr[Byte], size: size_t, pos: size_t)(using
        Zone
    ): Ptr[ZSTD_inBuffer_s] =
      val ____ptr = apply()
  ...
  object ZSTD_outBuffer:
    given _tag: Tag[ZSTD_outBuffer] =
      Tag.materializeCStruct3Tag[Ptr[Byte], size_t, size_t]
    def apply()(using Zone): Ptr[ZSTD_outBuffer] =
      scala.scalanative.unsafe.alloc[ZSTD_outBuffer](1)
    def apply(dst: Ptr[Byte], size: size_t, pos: size_t)(using
        Zone
    ): Ptr[ZSTD_outBuffer] =
      val ____ptr = apply()
      (!____ptr).dst = dst
  ...
  object ZSTD_outBuffer_s:
    given _tag: Tag[ZSTD_outBuffer_s] =
      Tag.materializeCStruct3Tag[Ptr[Byte], size_t, size_t]
    def apply()(using Zone): Ptr[ZSTD_outBuffer_s] =
      scala.scalanative.unsafe.alloc[ZSTD_outBuffer_s](1)
    def apply(dst: Ptr[Byte], size: size_t, pos: size_t)(using
        Zone
    ): Ptr[ZSTD_outBuffer_s] =
      val ____ptr = apply()
      (!____ptr).dst = dst
      (!____ptr).size = size
      (!____ptr).pos = pos
...
private[zstd] object extern_functions:
  ...
  /** ZSTD_CCtx_loadDictionary() : Requires v1.4.0+ Create an internal CDict
    * from `dict` buffer. Decompression will have to use same dictionary.
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_CCtx_loadDictionary(
      cctx: Ptr[ZSTD_CCtx],
      dict: Ptr[Byte],
      dictSize: size_t
  ): size_t = extern

  /** ZSTD_CCtx_refCDict() : Requires v1.4.0+ Reference a prepared dictionary,
    * to be used for all future compressed frames. Note that compression
    * parameters are enforced from within CDict, and supersede any compression
    * parameter previously set within CCtx. The parameters ignored are labelled
    * as "superseded-by-cdict" in the ZSTD_cParameter enum docs. The ignored
    * parameters will be used again if the CCtx is returned to no-dictionary
    * mode. The dictionary will remain valid for future compressed frames using
    * same CCtx.
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_CCtx_refCDict(cctx: Ptr[ZSTD_CCtx], cdict: Ptr[ZSTD_CDict]): size_t =
    extern

  /** ZSTD_CCtx_refPrefix() : Requires v1.4.0+ Reference a prefix (single-usage
    * dictionary) for next compressed frame. A prefix is **only used once**.
    * Tables are discarded at end of frame (ZSTD_e_end). Decompression will need
    * same prefix to properly regenerate data. Compressing with a prefix is
    * similar in outcome as performing a diff and compressing it, but performs
    * much faster, especially during decompression (compression speed is tunable
    * with compression level).
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_CCtx_refPrefix(
      cctx: Ptr[ZSTD_CCtx],
      prefix: Ptr[Byte],
      prefixSize: size_t
  ): size_t = extern
  ...
  /** ZSTD_CCtx_reset() : There are 2 different things that can be reset,
    * independently or jointly : - The session : will stop compressing current
    * frame, and make CCtx ready to start a new one. Useful after an error, or
    * to interrupt any ongoing compression. Any internal data not yet flushed is
    * cancelled. Compression parameters and dictionary remain unchanged. They
    * will be used to compress next frame. Resetting session never fails. - The
    * parameters : changes all parameters back to "default". This also removes
    * any reference to any dictionary or external sequence producer. Parameters
    * can only be changed between 2 sessions (i.e. no compression is currently
    * ongoing) otherwise the reset fails, and function returns an error value
    * (which can be tested using ZSTD_isError()) - Both : similar to resetting
    * the session, followed by resetting parameters.
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  ...
  /** ZSTD_CCtx_setParameter() : Set one compression parameter, selected by enum
    * ZSTD_cParameter. All parameters have valid bounds. Bounds can be queried
    * using ZSTD_cParam_getBounds(). Providing a value beyond bound will either
    * clamp it, or trigger an error (depending on parameter). Setting a
    * parameter is generally only possible during frame initialization (before
    * starting compression). Exception : when using multi-threading mode
    * (nbWorkers >= 1), the following parameters can be updated _during_
    * compression (within same frame): => compressionLevel, hashLog, chainLog,
    * searchLog, minMatch, targetLength and strategy. new parameters will be
    * active for next job only (after a flush()).
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_CCtx_setParameter(
      cctx: Ptr[ZSTD_CCtx],
      param: ZSTD_cParameter,
      value: CInt
  ): size_t = extern

  /** ZSTD_CCtx_setPledgedSrcSize() : Total input data size to be compressed as
    * a single frame. Value will be written in frame header, unless if
    * explicitly forbidden using ZSTD_c_contentSizeFlag. This value will also be
    * controlled at end of frame, and trigger an error if not respected.
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_CCtx_setPledgedSrcSize(
      cctx: Ptr[ZSTD_CCtx],
      pledgedSrcSize: CUnsignedLongLong
  ): size_t = extern
  ...
  /** ZSTD_DCtx_loadDictionary() : Requires v1.4.0+ Create an internal DDict
    * from dict buffer, to be used to decompress all future frames. The
    * dictionary remains valid for all future frames, until explicitly
    * invalidated, or a new dictionary is loaded.
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_DCtx_loadDictionary(
      dctx: Ptr[ZSTD_DCtx],
      dict: Ptr[Byte],
      dictSize: size_t
  ): size_t = extern

  /** ZSTD_DCtx_refDDict() : Requires v1.4.0+ Reference a prepared dictionary,
    * to be used to decompress next frames. The dictionary remains active for
    * decompression of future frames using same DCtx.
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_DCtx_refDDict(dctx: Ptr[ZSTD_DCtx], ddict: Ptr[ZSTD_DDict]): size_t =
    extern
  ...
  /** ZSTD_DCtx_refPrefix() : Requires v1.4.0+ Reference a prefix (single-usage
    ...
    * compression. Prefix is **only used once**. Reference is discarded at end
    * of frame. End of frame is reached when ZSTD_decompressStream() returns 0.
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_DCtx_refPrefix(
      dctx: Ptr[ZSTD_DCtx],
      prefix: Ptr[Byte],
      prefixSize: size_t
  ): size_t = extern
  ...
  /** ZSTD_DCtx_reset() : Return a DCtx to clean state. Session and parameters
    * can be reset jointly or separately. Parameters can only be reset when no
  ...
  /** ZSTD_DCtx_setParameter() : Set one compression parameter, selected by enum
    * ZSTD_dParameter. All parameters have valid bounds. Bounds can be queried
    * using ZSTD_dParam_getBounds(). Providing a value beyond bound will either
    * clamp it, or trigger an error (depending on parameter). Setting a
    * parameter is only possible during frame initialization (before starting
    * decompression).
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_DCtx_setParameter(
      dctx: Ptr[ZSTD_DCtx],
  ...
  /** ZSTD_compress2() : Behave the same as ZSTD_compressCCtx(), but compression
    * parameters are set using the advanced API. ZSTD_compress2() always starts
    * a new frame. Should cctx hold data from a previously unfinished frame,
    * everything about it is forgotten. - Compression parameters are pushed into
    * CCtx before starting compression, using ZSTD_CCtx_set*() - The function is
    * always blocking, returns when compression is completed. Hint : compression
    * runs faster if `dstCapacity` >= `ZSTD_compressBound(srcSize)`.
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  ...
  /** ZSTD_compressCCtx() : Same as ZSTD_compress(), using an explicit
    * ZSTD_CCtx. Important : in order to behave similarly to `ZSTD_compress()`,
    * this function compresses at requested compression level, __ignoring any
    * other parameter__ . If any advanced parameter was set using the advanced
    * API, they will all be reset. Only `compressionLevel` remains.
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_compressCCtx(
      cctx: Ptr[ZSTD_CCtx],
      dst: Ptr[Byte],
      dstCapacity: size_t,
      src: Ptr[Byte],
  ...
  /** Alternative for ZSTD_compressStream2(zcs, output, input, ZSTD_e_continue).
    * NOTE: The return value is different. ZSTD_compressStream() returns a hint
    * for the next read size (if non-zero and not an error).
    * ZSTD_compressStream2() returns the minimum nb of bytes left to flush (if
    * non-zero and not an error).
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_compressStream(
  ...
  /** ZSTD_compressStream2() : Requires v1.4.0+ Behaves about the same as
    ...
    * using ZSTD_CCtx_set*() - Compression parameters cannot be changed once
    * compression is started (save a list of exceptions in multi-threading mode)
    * \- output->pos must be <= dstCapacity, input->pos must be <= srcSize -
    * output->pos and input->pos will be updated. They are guaranteed to remain
    * below their respective limit. - endOp must be a valid directive - When
    * nbWorkers==0 (default), function is blocking : it completes its job before
    * returning to caller. - When nbWorkers>=1, function is non-blocking : it
    * copies a portion of input, distributes jobs to internal worker threads,
    * flush to output whatever is available, and then immediately returns, just
    * indicating that there is some data remaining to be flushed. The function
    * nonetheless guarantees forward progress : it will return only after it
    * reads or write at least 1+ byte. - Exception : if the first call requests
    * a ZSTD_e_end directive and provides enough dstCapacity, the function
    * delegates to ZSTD_compress2() which is always blocking. -
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_compressStream2(
      cctx: Ptr[ZSTD_CCtx],
      output: Ptr[ZSTD_outBuffer],
      input: Ptr[ZSTD_inBuffer],
  ...
  /** ZSTD_compress_usingCDict() : Compression using a digested Dictionary.
    * Recommended when same dictionary is used multiple times. Note :
    * compression level is _decided at dictionary creation time_, and frame
    * parameters are hardcoded (dictID=yes, contentSize=yes, checksum=no)
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_compress_usingCDict(
      cctx: Ptr[ZSTD_CCtx],
      dst: Ptr[Byte],
      dstCapacity: size_t,
      src: Ptr[Byte],
      srcSize: size_t,
      cdict: Ptr[ZSTD_CDict]
  ): size_t = extern
  ...
  /** ZSTD_createCDict() : When compressing multiple messages or blocks using
    * the same dictionary, it's recommended to digest the dictionary only once,
    * since it's a costly operation. ZSTD_createCDict() will create a state from
    * digesting a dictionary. The resulting state can be used for future
    * compression operations with very limited startup cost. ZSTD_CDict can be
    * created once and shared by multiple threads concurrently, since its usage
    * is read-only. can be released after ZSTD_CDict creation, because its
    * content is copied within CDict. Note 1 : Consider experimental function
    * `ZSTD_createCDict_byReference()` if you prefer to not duplicate content.
    * Note 2 : A ZSTD_CDict can be created from an empty , in which case the
    * only thing that it transports is the . This can be useful in a pipeline
    * featuring ZSTD_compress_usingCDict() exclusively, expecting a ZSTD_CDict
    * parameter with any data, including those without a known dictionary.
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_createCDict(
      dictBuffer: Ptr[Byte],
      dictSize: size_t,
      compressionLevel: CInt
  ): Ptr[ZSTD_CDict] = extern
  ...
  /** ZSTD_createDDict() : Create a digested dictionary, ready to start
    * decompression operation without startup delay. dictBuffer can be released
    * after DDict creation, as its content is copied inside DDict.
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_createDDict(
      dictBuffer: Ptr[Byte],
      dictSize: size_t
  ): Ptr[ZSTD_DDict] = extern
  ...
  /** ZSTD_decompress() : `compressedSize` : must be the _exact_ size of some
    * number of compressed and/or skippable frames. `dstCapacity` is an upper
    * bound of originalSize to regenerate. If user cannot imply a maximum upper
    * bound, it's better to use streaming mode to decompress data.
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  ...
  /** ZSTD_decompressStream() : Streaming decompression function. Call
    * repetitively to consume full input updating it as necessary. Function will
    * update both input and output `pos` fields exposing current state via these
    * fields: - `input.pos < input.size`, some input remaining and caller should
    * provide remaining input on the next call. - `output.pos < output.size`,
    * decoder finished and flushed all remaining buffers. - `output.pos ==
    * output.size`, potentially uncflushed data present in the internal buffers,
    * call ZSTD_decompressStream() again to flush remaining data to output. Note
    * : with no additional input, amount of data flushed <= ZSTD_BLOCKSIZE_MAX.
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_decompressStream(
  ...
  /** ZSTD_decompress_usingDDict() : Decompression using a digested Dictionary.
    ...
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_decompress_usingDDict(
      dctx: Ptr[ZSTD_DCtx],
      dst: Ptr[Byte],
      dstCapacity: size_t,
      src: Ptr[Byte],
      srcSize: size_t,
      ddict: Ptr[ZSTD_DDict]
  ): size_t = extern

  /** ZSTD_decompress_usingDict() : Decompression using a known Dictionary.
    * Dictionary must be identical to the one used during compression. Note :
    * This function loads the dictionary, resulting in significant startup
  ...
  /** ZSTD_findFrameCompressedSize() : Requires v1.4.0+ `src` should point to
    * the start of a ZSTD frame or skippable frame. `srcSize` must be >= first
    * frame size
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_findFrameCompressedSize(src: Ptr[Byte], srcSize: size_t): size_t =
    extern
  ...
  /** ZSTD_getDecompressedSize() : NOTE: This function is now obsolete, in favor
    * of ZSTD_getFrameContentSize(). Both functions work the same way, but
    * ZSTD_getDecompressedSize() blends "empty", "unknown" and "error" results
    * to the same return value (0), while ZSTD_getFrameContentSize() gives them
    * separate return values.
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_getDecompressedSize(
      src: Ptr[Byte],
      srcSize: size_t
  ...
  /** ZSTD_getDictID_fromDict() : Requires v1.4.0+ Provides the dictID stored
    * within dictionary. if
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_getDictID_fromDict(dict: Ptr[Byte], dictSize: size_t): CUnsignedInt =
    extern

  /** ZSTD_getDictID_fromFrame() : Requires v1.4.0+ Provides the dictID required
    * to decompressed the frame stored within `src`. If
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_getDictID_fromFrame(src: Ptr[Byte], srcSize: size_t): CUnsignedInt =
    extern
  ...
  /** Equivalent to:
    ...
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_initCStream(zcs: Ptr[ZSTD_CStream], compressionLevel: CInt): size_t =
    extern
  ...
  /** ZSTD_sizeof_*() : Requires v1.4.0+ These functions give the _current_
    * memory usage of selected object. Note that object memory usage can evolve
  ...
  /** ZSTD_versionString() : Return runtime library version, like "1.4.5".
    * Requires v1.3.0+.
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
  ...
  private[zstd] def __sn_wrap_zstd_ZSTD_cParam_getBounds(
      cParam: ZSTD_cParameter,
      __return: Ptr[ZSTD_bounds]
  ): Unit = extern

  private[zstd] def __sn_wrap_zstd_ZSTD_dParam_getBounds(
...
object functions:
  ...
  /** ZSTD_cParam_getBounds() : All parameters must belong to an interval with
    * lower and upper bounds, otherwise they will either trigger an error or be
  ...
  /** ZSTD_cParam_getBounds() : All parameters must belong to an interval with
    ...
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_cParam_getBounds(cParam: ZSTD_cParameter)(
      __return: Ptr[ZSTD_bounds]
  ): Unit =
    __sn_wrap_zstd_ZSTD_cParam_getBounds(cParam, __return)

  /** ZSTD_dParam_getBounds() : All parameters must belong to an interval with
    * lower and upper bounds, otherwise they will either trigger an error or be
    * automatically clamped.
    *
    * [bindgen] header:
    * /Users/velvetbaldmime/Library/Caches/sbt-vcpkg/vcpkg/packages/zstd_arm64-osx/include/zstd.h
    */
  def ZSTD_dParam_getBounds(dParam: ZSTD_dParameter)(
      __return: Ptr[ZSTD_bounds]
  ): Unit =
